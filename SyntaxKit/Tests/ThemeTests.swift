...
internal class ThemeTests: XCTestCase {
    ...
    func testLoading() {
        if let tomorrow = manager.theme(withIdentifier: "Tomorrow") {
            ...
            assertEqualColors(Color(hex: "#666969"), tomorrow.attributes["constant.other"]?[NSAttributedStringKey.foregroundColor] as? Color)
            assertEqualColors(Color(hex: "#4271AE"), tomorrow.attributes["keyword.other.special-method"]?[NSAttributedStringKey.foregroundColor] as? Color)
        } else {
    ...
    func testComplexTheme() {
        if let solarized = manager.theme(withIdentifier: "Solarized") {
            ...
            assertEqualColors(Color(hex: "#2aa198"), solarized.attributes["string.quoted.double"]?[NSAttributedStringKey.foregroundColor] as? Color)
            assertEqualColors(Color(hex: "#2aa198"), solarized.attributes["string.quoted.single"]?[NSAttributedStringKey.foregroundColor] as? Color)
        } else {
